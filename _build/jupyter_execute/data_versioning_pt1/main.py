...
# # Cache Design
# 
# Unfortunately, DVC does not provide tooling necessary for such data caching. So let's think of potential use cases and design our cache more accuratel
# 
...
# |:-------------------|:-------------------|:-------------------|
# | **Memory - Low**   | Hard to compute, easy to store. <br> Examples: *class indicies, <br> embeddings, descriptors etc.* <br><br> Cached. | Info extacted from function arguments. <br> Examples: *IDs, tags, labels extracted <br> from file paths.* <br><br> Not cached |  
...
# 
# The first group consists of some lightweight objects which are extracted from the raw data using ML models or other slow algorithms. This data is typically used for sampling and/or filtering further in the data pipeline, so we need be able to prefetch such data from the cache into memory. On the other hand the second group includes data which cannot be fit into memory in large numbers and must be loaded lazily. The data record may consist of both types of data so our cache should support both as well. 
# 
# These are four main requirements to our data preprocessing cache. There are plenty of Python tools for caching but seems like they fall short in one way or another. I haven't been able to find disk caching tools with appropriate invalidation strategies - most of them use TTL of some kind, which is not what we want, as our cache can potentially live for months. Also two-level caching with lazy reads is kinda specific requirement, so I haven't been able to find anything like that as well. For this reason we'll resort to writing our own solution. We will definetely face fun engineering challenges there. But that's it for today.
# 
# Until next time.
