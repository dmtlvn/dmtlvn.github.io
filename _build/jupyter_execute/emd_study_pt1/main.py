...
# 
# For everything to make sense, these vectors $p_k$ must be coplanar to a $(i_k, j_k)$ coordinate plane, where $i_k$ and $j_k$ are indicies of bins between which the mass exchange is performed on the step $k$. Nothings beats a good visualization, so here's a nice 3D example:
...
# 
# 3. More so, we can take all sub-steps $p_{ki}$ and shuffle them in any way, which preserves continuity of the the whole transport plan and ensures that the new plan still adds up to $P$. This is true because by splitting $p_k$ we just split the mass transferred, and by shuffling them we change only at which step this smaller mass is transferred, but the destination stays the same, thus the whole flow between each pair of bins stays the same.
# 
...
# 
# These two plans are optimal and correspond to EMD $\approx$ 0.593, but the fact that they look completely different geometrically raises a guestion: what is common between them, which guaranties the equal minimal cost?
...
# 
# Let's plot EMD agains $\|P\|_1$ for random histograms with various number of bins:
...
# But for the completeness let's take a look at some other distribution similarity measures in realation to EMD and check which upper and lower bounds the induce on it. 
...
# 4. A total variation distance (a maximum absolute difference between histograms) is also sligtly concave but close 
...
# As you can see, other common metrics are either very similar to $L_1$ as upper bounds on EMD, or are harder to optimize, or more difficult to compute. We can think of a more complicated optimization schedule, where we use one metric, then switch for a better one, but then we start loosing a point: we want a simpler alternative to EMD. And while $L_1$-distance isn't something ground breaking on itself, it has these nice properties which favour it among other metrics.
...
# 
# As expected, the Euclidean distance performs the best in terms of the variational error. The error is reduced with the learning rate, which means that it comes mostly from the jittering near the minimum. The same results are for the cosine similarity as well, so they are ommited. 
# 
...
# Definetely not the best function to use along gradient methods, with lots of descrete jumps which cause that jaggedness of error curve. I bet the actual trajectory zig-zags like crazy as it tries to move along a slanted line with discrete steps. Also a fun fact is that the derivative has more terms for the bins with smaller indicies. This makes a gradient for first bin more noisy then for the last one, which is somewhat arbitrary. 
# 
...
# 1. Any similarity measure induces an upper bound on any other similarity measure and proved $L_1$ and Kolmogorov-Smirnov upper bounds on EMD, which are optimal in sense that they are linear and uniform. 
...
# 
# All of that to just conlude, that if you need to match histograms using gradient methods, then you can just use MSE or MAE and don't overthink things when unnecessary.
# 
